...
# docker build -t mesh/modelstorage -f docker/ModelStorage.Dockerfile .
...
RUN mkdir out \
    ...
    && cmake --build . --target ModelsStorageSvc -j 8

# Copy to the final (second stage) image
FROM scratch as runtime
LABEL description="Mesh - Model Storage Service"

# Main files
COPY --from=build /mesh/out/src/Services/Storage/ModelsStorageSvc/ModelsStorageSvc /mesh/out/src/Services/Storage/ModelsStorageSvc/ModelsStorageSvc
...
# Service start
CMD ["/mesh/out/src/Services/Storage/ModelsStorageSvc/ModelsStorageSvc"]
