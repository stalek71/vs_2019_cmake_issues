...
# To build it use the command shown below (from the root folder). Don't overlook dot (.) at the end:
# docker build -t mesh/calc -f docker/Calculation.Dockerfile .
...
RUN mkdir out \
    ...
    && cmake --build . --target CalculationSvc -j 8

# Copy to the final (second stage) image
FROM scratch as runtime
LABEL description="Mesh - Calculation Service"

# Main files
COPY --from=build /mesh/out/src/Services/CalculationSvc/CalculationSvc /mesh/out/src/Services/CalculationSvc/CalculationSvc
...
# Service start
CMD ["/mesh/out/src/Services/CalculationSvc/CalculationSvc"]
