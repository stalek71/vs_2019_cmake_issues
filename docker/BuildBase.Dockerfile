...
RUN apk update && \
    apk add --no-cache \
    autoconf build-base binutils cmake curl file gcc g++ git libgcc libtool linux-headers make musl-dev musl tar unzip wget ninja zip pkgconf libstdc++

# GLIBC - replaces libc6-compat
RUN cd /tmp && mkdir glibpkg && cd glibpkg && \
